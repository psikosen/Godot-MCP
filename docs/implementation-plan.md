...
- **Status**: Audio bus introspection delivered via `list_audio_buses`, mutation command `configure_audio_bus`, node-level authoring with `author_audio_stream_player`, and interactive music graph tooling through `author_interactive_music_graph` for clip layering and transitions.
